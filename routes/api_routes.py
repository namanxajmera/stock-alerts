...
from utils.validators import validate_ticker_symbol, validate_period, ValidationError
...
def get_stock_data(ticker: str, period: str) -> Any:
    ...
    logger.info(f"Stock data request from {client_ip}: ticker={ticker}, period={period}")
    
    try:
        # Validate ticker symbol with enhanced security checks
        is_valid_ticker, validated_ticker = validate_ticker_symbol(ticker)
        if not is_valid_ticker:
            logger.warning(f"Invalid ticker from {client_ip}: {ticker} - {validated_ticker}")
        ...
        if not is_valid_period:
            logger.warning(f"Invalid period from {client_ip}: {period} - {validated_period}")
        ...
        result, status_code = stock_service.calculate_metrics(validated_ticker, validated_period)
        
        # Log successful request
        if status_code == 200:
            logger.info(f"Successful data request for {validated_ticker}/{validated_period} from {client_ip}")
        else:
            logger.warning(f"Stock service error for {validated_ticker}/{validated_period}: {result.get('error', 'Unknown error')}")
        ...
        logger.error(f"Unexpected error processing request from {client_ip}: {e}", exc_info=True)
